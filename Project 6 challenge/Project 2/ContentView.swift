...
struct ContentView: View {
    ...
    @State private var animationFadeAmount = 2.0
                ...
                VStack(spacing: 15) {
                    ...
                    ForEach(0..<3) { number in
                        Button {
                            ...
                            animationAmount += 360
                            withAnimation(
                                .interpolatingSpring(stiffness: 10, damping: 4)
                            ) {
                                animationAmount += 360
                                animationFadeAmount += 1
                            }
                        } label: {
                            FlagImage(image: countries[number])
                        }
                        .rotation3DEffect(.degrees(selectedButtonIndex == number ? animationAmount : 0), axis: (x: 0, y: 1, z: 0))
                        .opacity(selectedButtonIndex == number || selectedButtonIndex == 3 ? 1 : 0.5)
                        .scaleEffect(selectedButtonIndex == number || selectedButtonIndex == 3 ? 1 : 0.7)
                    }
                }
